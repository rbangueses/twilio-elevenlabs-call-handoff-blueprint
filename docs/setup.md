...
| `ROUTING_MODE` | `taskrouter` or `direct`; use `/studio_escalate` for the Studio return path. |
...
| `DIRECT_TRANSFER_MODE` / `DIRECT_TRANSFER_TO` / `DIRECT_TRANSFER_FROM` / `DIRECT_HOLD_URL` | Direct cold-dial or warm-conference configuration. |
...
3. For the direct and TaskRouter paths, point the Voice webhook to `/voice`. For the Studio path, point it to the Studio Flow, whose TwiML Redirect calls `/studio_voice`.
